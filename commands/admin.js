  ...
  async execute(client, interaction) {
    ...
    if (interaction.options.getString("command") == "dall") {
      for (let i = 0; i < guildflights.length; i++) {
        await flightdb.delete(guildflights[i]);
      }
      ...
      interaction.reply({
        embeds: [
          respond(
            "Deleted all flights!",
            "All flights have been removed from the list!"
          ),
        ],
        ephemeral: true,
      });
      ...
      if (interaction.options.getString("flightnumber") == undefined) {
        interaction.reply({
          embeds: [
            respond(
              "An Error encourred!",
              "The opional field **flightnumber** hasn't been filled out!"
            ),
          ],
          ephemeral: true,
        });
      ...
      interaction.reply({
        embeds: [
          respond(
            "Deleted flight " + interaction.options.getString("flightnumber"),
            interaction.options.getString("flightnumber") +
